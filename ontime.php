     ...
     <div class="container">
			...
			<div class="mt-5 text-center sort-title">
     		...
     		<div class="p-5">
     		    <hr>
			<?php 
				  $sql="SELECT class FROM onlinetime ";
				$query=mysqli_query($con,$sql);
				while($row=mysqli_fetch_assoc($query))
				{
			?>
     		     <a href="onlinetimetable.php?class=<?php echo $row['class'];?>" class="btn btn-outline-info btn-lg" style="margin:5px;"><?php echo $row['class'];?></a>
				<?php } ?>
     		    <hr>
     	        </div>
     </div>
